...
class NewBufferedTokenizer
  ...
  def extract(data)
    ...
    if @size_limit && @input.size > @size_limit
      raise BufferFullException, "#{self.inspect} reached limit of #{@size_limit}"
    end
    
    ents = []
    while idx = @input.index(@delimeter)
      token = @input.slice!(0, idx + @delimeter_size)
      ents << token[0, idx]
    end
    ents
  end
...
if __FILE__ == $0
  ...
  puts '# new: ' + NewBufferedTokenizer.new("\n").extract(data * 3).inspect
  puts '# old: ' + BufferedTokenizer.new("\n").extract(data * 3).inspect

  Benchmark.bmbm do |results|
    results.report("new:") { 
      b = NewBufferedTokenizer.new("\n") 
      TESTS.times { 
        b.extract(data)
      }
    }
